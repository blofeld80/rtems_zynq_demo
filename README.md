...
Before we start, change the board to `xilinx_zynq_picozed` (in `config.sh`), then build and install RTEMS / libbsd by running the following script:

```
blofeld@ubuntu:~/demo$ ./2_3_0_build_rtems_libbsd.sh 
```
...
```
blofeld@ubuntu:~/demo$ ./2_3_1_create_toolchain_file.sh
```
